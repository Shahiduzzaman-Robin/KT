  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#f4faff] px-4 py-6 md:px-8">
      ...
      <div className="mx-auto flex max-w-[1700px] gap-5 px-0 md:px-0">
        ...
        <header className="relative z-20 rounded-xl bg-white/90 p-5 shadow-[0_12px_40px_rgba(0,31,42,0.06)] backdrop-blur">
        ...
        <section className="rounded-xl bg-[#e6f6ff] p-5">
          <div className="grid gap-2 md:grid-cols-[1fr_220px_auto]">
            ...
            />
            <button className="rounded-lg bg-gradient-to-br from-[#00694b] to-[#008560] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-95" type="button" onClick={loadLedgers}>
              Refresh
            </button>
          </div>

          <div className="mt-4 grid gap-2 text-sm md:grid-cols-5">
            <p className="rounded-lg bg-white px-3 py-2 text-[#3d4a43]">Total: <span className="font-semibold text-[#001f2a]">{groupedStats.total}</span></p>
            <p className="rounded-lg bg-white px-3 py-2 text-[#3d4a43]">Customer: <span className="font-semibold text-[#001f2a]">{groupedStats.customer}</span></p>
            <p className="rounded-lg bg-white px-3 py-2 text-[#3d4a43]">Supplier: <span className="font-semibold text-[#001f2a]">{groupedStats.supplier}</span></p>
            <p className="rounded-lg bg-white px-3 py-2 text-[#3d4a43]">Employee: <span className="font-semibold text-[#001f2a]">{groupedStats.employee}</span></p>
            <p className="rounded-lg bg-white px-3 py-2 text-[#3d4a43]">Other: <span className="font-semibold text-[#001f2a]">{groupedStats.other}</span></p>
          </div>
        </section>
                ...
                {ledgers.map((ledger, index) => (
                  <tr key={ledger._id} className={`${index % 2 ? 'bg-[#f8fcff]' : 'bg-white'} align-top transition hover:bg-[#eaf7ff]`}>
                    ...
                    <td className="px-4 py-4">
                      <span className="rounded-lg bg-[#d9f2ff] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[#005139]">
                        {ledger.type}
                      </span>
                    </td>
